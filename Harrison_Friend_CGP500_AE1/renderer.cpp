	...
	void Renderer::RenderLoop()
	{
		...
		for (int m = 0; m<meshes.size(); ++m)
		{
			...
			if (constants)
			{
				...
				const Matrix4 scaleMatrix = Matrix4::scale(Vector3(scale.getX(), scale.getY(), scale.getZ()));
				const Matrix4 ScreenAspectRatio = Matrix4::scale(Vector3(1, kAspectRatio, 1));
				const Matrix4 transMatrix = Matrix4::translation(translation);
				constants->m_WorldViewProj = ToMatrix4Unaligned(ScreenAspectRatio* transMatrix * rotationMatrix* scaleMatrix);
